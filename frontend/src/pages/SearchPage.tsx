      ...
      <div className="max-w-7xl mx-auto">
        ...
        <form onSubmit={handleSubmit(performSearch)} className="space-y-4 mb-12">
          <div className="relative max-w-2xl mx-auto">
            <input {...register("query")} placeholder="Search by name, bio, or tags..." className="w-full px-6 py-4 rounded-full border border-gray-300 dark:border-gray-600 bg-white/70 dark:bg-gray-800/70 focus:ring-2 focus:ring-primary outline-none shadow-lg" />
            <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 btn-primary rounded-full h-12 w-24">Search</button>
          </div>

          <div className="text-center">
            <button type="button" onClick={() => setShowFilters(!showFilters)} className="text-primary dark:text-primary-light font-medium">{showFilters ? 'Hide' : 'Show'} Filters</button>
          </div>

          {showFilters && (
            <div className="max-w-4xl mx-auto p-6 bg-white/50 dark:bg-gray-800/50 backdrop-blur-md rounded-2xl shadow-inner-lg border border-gray-200/30 dark:border-gray-700/30 grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Followers */}
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Follower Count</label>
                <div className="flex gap-4 mt-1">
                  <input type="number" {...register("minFollowers")} placeholder="Min" className="input w-full" />
                  <input type="number" {...register("maxFollowers")} placeholder="Max" className="input w-full" />
                </div>
              </div>
              ...
              <div>
                ...
                <Controller
                  ...
                  render={({ field }) => {
                    ...
                    return (
                      <div className="mt-1 flex flex-wrap items-center w-full px-2 py-1 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50/70 dark:bg-gray-700/70">
                        {field.value.map((category, index) => (
                          <div key={index} className="flex items-center bg-primary/20 text-primary-dark dark:text-primary-light text-sm font-medium mr-2 mb-1 px-2 py-1 rounded-full">
                            <span>{category}</span>
                            <button type="button" onClick={() => field.onChange(field.value.filter((c) => c !== category))} className="ml-2 text-red-500 hover:text-red-700">&times;</button>
                          </div>
                        ))}
                        <div className="flex-grow flex items-center">
                          <input type="text" value={categoryInput} onChange={(e) => setCategoryInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addCategory(); } }} className="flex-grow bg-transparent outline-none" placeholder="Add categories..." />
                    ...
                    return (
                      <div className="mt-1 flex flex-wrap items-center w-full px-2 py-1 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50/70 dark:bg-gray-700/70">
                        {field.value.map((tag, index) => (
                          <div key={index} className="flex items-center bg-primary/20 text-primary-dark dark:text-primary-light text-sm font-medium mr-2 mb-1 px-2 py-1 rounded-full">
                            <span>{tag}</span>
                            <button type="button" onClick={() => field.onChange(field.value.filter((t) => t !== tag))} className="ml-2 text-red-500 hover:text-red-700">&times;</button>
                          </div>
                        ))}
                        <div className="flex-grow flex items-center">
                          <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addTag(); } }} className="flex-grow bg-transparent outline-none" placeholder="Add tags..." />
